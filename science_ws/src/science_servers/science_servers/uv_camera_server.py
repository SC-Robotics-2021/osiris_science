...
class UVCameraServer(Node):
    ...
    def __init__(self):
        ...
        Class constructor.
        """
        super().__init__('uv_camera_server')
        self.service = self.create_service(SetBool, '/osiris/science/uv_camera/cmd', self.receive_request)
        self.active = False
        self.publisher = self.create_publisher(CompressedImage, '/osiris/science/uv_camera/images',
    ...
    def receive_request(self, request, response):
        ...
        if self.active != request.data:
            ...
            if request.data:
                ...
                status = 'off'
            response.message = f'UV camera {status}'
        else:
            response.success = False
            response.message = 'UV camera already in requested state'
    ...
    def publish_frame(self):
        ...
        """
        if self.active:
            if self.count_subscribers('/osiris/science/uv_camera/images') > 0:
                success, frame = self.camera.read()
                if success:
                    self.get_logger().info('Publishing UV camera frame')
...
def main(args=None):
    ...
    uv_camera_server = UVCameraServer()
    try:
        rclpy.spin(uv_camera_server)
    except Exception as e:
        uv_camera_server.get_logger().info(f'{e})')
    except KeyboardInterrupt:
        print('\n')
    finally:
        uv_camera_server.camera.release()
        uv_camera_server.destroy_node()
        rclpy.shutdown()
