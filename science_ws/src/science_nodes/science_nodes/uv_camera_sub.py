...
class UVCameraSub(Node):
    ...
    def __init__(self):
        ...
        media_path = os.path.expanduser(f'~/Videos/osiris_science/uv_camera/')
        try:
    ...
    def receive_frame(self, frame):
        ...
        if self.record:
            if self.out is None:
                ...
                video_path = os.path.join(self.media_path, 'UV_Camera_' + now + '.mp4')
        ...
        if self.snapshot:
            now = datetime.now().strftime('%m-%d-%Y_%H:%M:%S')
            image_path = os.path.join(self.media_path, 'UV_Camera_' + now + '.jpeg')
