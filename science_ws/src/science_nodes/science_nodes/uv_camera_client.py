...
class UVCameraClient(Node):
    ...
    def __init__(self):
        ...
        self.cli = self.create_client(SetBool, '/osiris/science/uv_camera/cmd')
            ...
            if self.future.done():
                try:
                    ...
                    self.get_logger().info(
                        f'Result of /osiris/science/uv_camera/cmd:'
...
def main(args=None):
    ...
    try:
        while proceed != '':
            status = int(input('Enter 1 to turn the UV camera on, or 0 to turn it off: '))
            while status < 0 or status > 1:
                status = int(input('Enter 1 to turn the UV camera on, or 0 to turn it off: '))
            uv_camera_client.send_request(bool(status))
            while rclpy.ok():
                done = uv_camera_client.run_node()
                if done:
                    break
