...
class BrushClient(Node):
    """
    Subclass of ROS2 Node. Issues control commands to brush component.
    """
    def __init__(self):
        """
        ...
        self.client = self.create_client(SetBool, '/osiris/science/brush/cmd')
    ...
    def send_request(self, state):
        ...
        while not self.client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('Brush service not available, waiting again...')
        ...
        while rclpy.ok():
            ...
            if self.future.done():
                try:
                    ...
                    self.get_logger().info(f'Result of /osiris/science/brush/cmd:' +
...
def main(args=None):
    ...
    try:
        while proceed != '':
            state = input('Enter 1 to turn the brush on, or 0 to turn it off: ')
            while state != '0' or state != '1':
                state = input('Enter 1 to turn the brush on, or 0 to turn it off: ')
            brush_client.send_request(bool(state))
            brush_client.run()
