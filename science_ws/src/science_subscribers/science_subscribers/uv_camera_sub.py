...
class UVCameraSub(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(CompressedImage, '/osiris/science/uv_camera/images',
    ...
    def receive_frame(self, frame):
        ...
        if self.snapshot:
            now = datetime.now().strftime('%m-%d-%Y_%H:%M:%S')
            cv2.imwrite(os.path.join(self.media_path, 'UV_Camera_' + now + '.jpeg'), self.frame)
            self.snapshot = False
