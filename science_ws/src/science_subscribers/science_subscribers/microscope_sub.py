...
class MicroscopeSub(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(CompressedImage, '/osiris/science/microscope/images',
    ...
    def receive_frame(self, frame):
        ...
        self.get_logger().info('Receiving microscope frame')  # logging is done after recording to ensure data retention
        if self.snapshot:
            now = datetime.now().strftime('%m-%d-%Y_%H:%M:%S')
            cv2.imwrite(os.path.join(self.media_path, 'Microscope_' + now + '.jpeg'), self.frame)
            self.snapshot = False
