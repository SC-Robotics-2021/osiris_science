...
class IRCameraSub(Node):
    ...
    def __init__(self):
        ...
        self.media_path = os.path.join(os.path.dirname(os.path.realpath(sys.argv[0])), 'science_recordings', 'ir_camera')
        try:
            os.makedirs(self.media_path, exist_ok=True)
        except OSError as e:
            self.get_logger().info(f'{e}')
        now = datetime.now().strftime('%m-%d-%Y_%H:%M:%S')
        self.out = cv2.VideoWriter(os.path.join(self.media_path, 'IR_Camera_' + now + '.mp4'),
