...
class UVLightClient(Node):
    """
    Subclass of ROS2 Node. Issues control commands to UV light component.
    """
    def __init__(self):
        """
        ...
        self.client = self.create_client(SetBool, '/osiris/science/uv_light/cmd')
        ...
        while rclpy.ok():
            ...
            if self.future.done():
                try:
                    ...
                    self.get_logger().info(f'Result of /osiris/science/uv_light/cmd:' +
...
def main(args=None):
    ...
    try:
        while proceed != '':
            state = input('Enter 1 to turn the UV light on, or 0 to turn it off: ')
            while state != '0' or state != '1':
                state = input('Enter 1 to turn the UV light on, or 0 to turn it off: ')
            uv_light_client.send_request(bool(state))
            uv_light_client.run()
