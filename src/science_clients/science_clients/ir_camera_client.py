...
class IRCameraClient(Node):
    """
    Subclass of ROS2 Node. Issues control commands to IR camera component.
    """
    def __init__(self):
        """
    ...
    def send_request(self, state):
        ...
        while not self.cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('IR camera service not available, waiting again...')
...
def main(args=None):
    ...
    try:
        while proceed != '':
            state = input('Enter 1 to turn the IR camera on, or 0 to turn it off: ')
            while state != '0' or state != '1':
                state = input('Enter 1 to turn the IR camera on, or 0 to turn it off: ')
            ir_camera_client.send_request(bool(state))
            ir_camera_client.run()
