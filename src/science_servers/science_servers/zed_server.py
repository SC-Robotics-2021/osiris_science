...
def main(args=None):
    ...
    zed_server = ZEDServer()
    try:
        rclpy.spin(zed_server)
    except Exception as e:
        zed_server.get_logger().info(f'{e})')
    except KeyboardInterrupt:
        print('\n')
    finally:
        zed_server.camera.release()
        zed_server.destroy_node()
        rclpy.shutdown()
